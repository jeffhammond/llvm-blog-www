...
This simple change represents a major milestone for the Flang.

This article will cover the almost 10 year journey of Flang from first concepts.
Including multiple rewrites, the adoption of LLVM's MLIR and Flang entering
the LLVM Project.
...
history of the Portland Group is important. Particularly their
engagements with US National Labs.
...
* **May 2017** The first source code release of Classic Flang as a separate
  ...
  from a PGI compiler and a new backend that generated LLVM IR.
...
Flang binary is named `flang`, just like the folder F18 now occupies in the LLVM monorepo.
...
* **May 2022** Kiran Chandramohan (Arm) [announces](https://www.youtube.com/watch?v=FoIjafZGDdE)
  at th European LLVM Developers' Meeting that Flang's OpenMP 1.1 support is close to complete.
  ...
  (this meant that `flang.sh` could be used as a drop in replacement for existing
  compilers)
...
the project. It became its own public branch `fir-dev`, that was latercontributed to the LLVM Project.
...
languages (Fortran array optimisations were an early cited use case for MLIR).

This lead languages such as Swift and Rust to create their own IRs that include
...
The next step is to convert the HLFIR into FIR:
```mlir
module attributes {<...>} {
...
Project. A month later, it was proposed that LLVM Flang should
share OpenMP support code with Flang.
...
This meant that Flang able to handle all the targets and tools that Clang could, without duplicating large amounts of code.

# Reflections on Flang

We are almost 10 years from the first announcement of what would become LLVM
Flang. In LLVM monorepo alone there have been close to 10,000 commits
...
> Collaboration between Linaro and Fujitsu on an active CI using the Fujitsu’s
> testsuite helped find several issues and make the compiler more robust, in
