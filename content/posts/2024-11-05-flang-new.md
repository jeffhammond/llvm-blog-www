...
Many of the features of modern Fortran require changes insider of the compiler itself.
One of the things that we (NVIDIA) learned during the first phase of LLVM Fortran
work (based on the open-source version of the PGI front-end) is that the changes
required to do modern Fortran effectively required a lot of changes, and it was
prudent to design a new front-end from scratch to address that need.
...
The other benefit is that having an LLVM Fortran compiler out there for everyone to use
is that there is a huge incentive for users to support it, if they know it's going to be
ubiquitous. What I have seen so far interacting with our Fortran users is that they are very
excited about LLVM Flang and were willing to commit to supporting it in their build
systems and CI systems, which has driven quality improvements in both the Flang compiler
and the applications.  It turns out, there is a lot of incorrect code out there
that wasn't detected by other compilers, but which is detected by Flang.
This is not unlike my experience in the past of finding incorrect code with Clang.
